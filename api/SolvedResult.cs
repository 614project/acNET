...
public record struct SolvedResult<T>(T? Result, Exception? Exception)
{
    /// <summary>
    ...
    /// </summary>
    /// <returns>결과</returns>
    public readonly T GetResultOrThrow() => this.Result ?? throw this.Exception ?? new NullReferenceException();
    /// <summary>
    /// Exception != null 이라면 예외를, 그렇지 않으면 Result를 .ToString() 한 결과를 가져옵니다.
    /// </summary>
    /// <returns>문자열</returns>
    public override readonly string ToString() => Exception is not null ? Exception.ToString() : Result?.ToString() ?? string.Empty;
}
